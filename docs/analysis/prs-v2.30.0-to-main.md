# All PRs merged between v2.30.0 and main

Date: 2026-07-21
Range: `v2.30.0..main` (main tip `f466b475e`)
Total: 186 PRs (KKP squash-merges; each row is one PR).

Full inventory for spotting e2e test candidates. Rows are sorted by testability:
deep-assessed candidates first, then heuristic shortlist, then likely/unlikely, with
dep-bumps / docs / chart-only pushed to the bottom.

## Columns

- **PR**: number, linked to GitHub.
- **Subsystems**: changed-file signals (master-ctrl, seed-ctrl, usercluster-ctrl, webhook,
  resources, api, installer, provider, sdk-types, has-gotest, dep-bump, docs, chart-only).
- **Files**: number of files changed (rough size).
- **Testability**: verdict for adding a pipeline e2e test.
  - `STRONG` / `MODERATE` / `WEAK` / `NOT-A-CANDIDATE` — deep-assessed (see
    `e2e-pipeline-candidates-v2.30.0-to-main.md` for the full reasoning).
  - `IMPLEMENTED` — already has a pipeline test (the UPB reference).
  - `COVERED` — already covered by another e2e job (gateway-api).
  - `shortlist (score N, not deep-assessed)` — passed the heuristic but was below the
    deep-assessment cut; worth a look.
  - `maybe (score N)` — weak heuristic signal.
  - `unlikely` — no behavioral signal for a seed-observable test.
  - `excluded: dep-bump | docs | chart-only` — no runtime behavior to assert.

## Tier summary

| Tier | Count |
|------|------:|
| STRONG (incl. seed-CP) | 2 |
| MODERATE | 5 |
| WEAK | 4 |
| NOT-A-CANDIDATE | 2 |
| IMPLEMENTED | 2 |
| COVERED (gateway-api e2e) | 4 |
| excluded (dep-bump / docs / chart-only) | 49 |
| shortlist / maybe / unlikely | 118 |

Recommendations and per-PR reasoning: see
[`e2e-pipeline-candidates-v2.30.0-to-main.md`](./e2e-pipeline-candidates-v2.30.0-to-main.md).
...
| [#15996](https://github.com/kubermatic/kubermatic/pull/15996) | fix Cilium NodeLocalDNS access with network policies | has-gotest, seed-ctrl | 2 | BUILD tier-B: STRONG, cluster-object |
...
| [#15691](https://github.com/kubermatic/kubermatic/pull/15691) | propagate upstream defaultValuesBlock changes on upgrade | has-gotest, sdk-types | 3 | IMPLEMENTED (appdef_defaultvalues_test.go) |
