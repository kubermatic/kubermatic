...
func (r *reconciler) Reconcile(ctx context.Context, resourceName string) error {
	r.logger.Debugf("Reconciling RBAC for %s", resourceName)
...
func (r *reconciler) ensureRBACClusterRole(ctx context.Context, resourceName string) error {
	...
	err = r.Get(ctx, ctrlruntimeclient.ObjectKey{Namespace: metav1.NamespaceAll, Name: resourceName}, clusterRole)
	if err != nil {
		// create Cluster Role if not exist
		if errors.IsNotFound(err) {
			if err := r.Create(ctx, defaultClusterRole); err != nil {
				return fmt.Errorf("failed to create the RBAC ClusterRole: %w", err)
			}
			r.logger.Debugf("Created a new ClusterRole %s", defaultClusterRole.Name)
			return nil
		}
		return err
	}
	...
	if !ClusterRoleMatches(clusterRole, defaultClusterRole) {
		...
		r.logger.Debugf("Updated the ClusterRole %s", clusterRole.Name)
	}
...
func (r *reconciler) ensureRBACClusterRoleBinding(ctx context.Context, resourceName string) error {
	...
	if err := r.Get(ctx, ctrlruntimeclient.ObjectKey{Namespace: metav1.NamespaceAll, Name: resourceName}, clusterRoleBinding); err != nil {
		if errors.IsNotFound(err) {
			if err := r.Create(ctx, defaultClusterBinding); err != nil {
				return fmt.Errorf("failed to create the RBAC ClusterRoleBinding: %w", err)
			}
			r.logger.Debugf("Created a new ClusterRoleBinding %s", defaultClusterBinding.Name)
			return nil
		}
		return err
	}
	...
	if !ClusterRoleBindingMatches(clusterRoleBinding, defaultClusterBinding) {
		if err := r.Update(ctx, defaultClusterBinding); err != nil {
			return fmt.Errorf("failed to update the RBAC ClusterRoleBinding: %w", err)
		}
		r.logger.Debugf("Updated the ClusterColeBinding %s", defaultClusterBinding.Name)
	}
	return nil
}
