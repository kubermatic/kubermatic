...
import (
	...
	"k8c.io/kubermatic/v2/pkg/resources/applications"
...
const (
	controllerName = "kkp-installation-controller"
)
...
func (r *reconciler) reconcile(ctx context.Context, log *zap.SugaredLogger, appInstallation *appkubermaticv1.ApplicationInstallation) error {
	...
	// get applicationVersion or uninstall Application if applicationDefinition has been removed or applicationVersion
	// does not exist in applicationDefinition
	...
	if err := r.seedClient.Get(ctx, types.NamespacedName{Name: appInstallation.Spec.ApplicationRef.Name}, applicationDef); err != nil {
		if apierrors.IsNotFound(err) {
			msg := fmt.Sprintf("ApplicationDefinition '%s' does not exist, removing applictionInstallation", applicationDef.Name)
			log.Info(msg)
			r.userRecorder.Event(appInstallation, corev1.EventTypeWarning, "ApplicationDefinitionNotFound", msg)
	...
	if !applicationDef.DeletionTimestamp.IsZero() {
		msg := fmt.Sprintf("ApplicationDefinition '%s' is being deleted,  removing applictionInstallation", applicationDef.Name)
		log.Info(msg)
		r.userRecorder.Event(appInstallation, corev1.EventTypeWarning, "ApplicationDefinitionDeleting", msg)
		return r.userClient.Delete(ctx, appInstallation)
	}
	...
	if err := r.getApplicationVersion(appInstallation, applicationDef, appVersion); err != nil {
		msg := fmt.Sprintf("failed to get applicationVersion: %s. removing Application", err)
		log.Infof(msg)
		r.userRecorder.Event(appInstallation, corev1.EventTypeWarning, "ApplicationVersionNotFound", msg)
		return r.userClient.Delete(ctx, appInstallation)
	}
...
// getApplicationVersion finds the applicationVersion defined appInstallation into the applicationDef and update the struct appVersion with it.
...
func (r *reconciler) handleInstallation(ctx context.Context, log *zap.SugaredLogger, appInstallation *appkubermaticv1.ApplicationInstallation) error {
	return r.appInstaller.Apply(ctx, log, r.userClient, appInstallation)
}
...
func (r *reconciler) handleDeletion(ctx context.Context, log *zap.SugaredLogger, appInstallation *appkubermaticv1.ApplicationInstallation) error {
	if kuberneteshelper.HasFinalizer(appInstallation, appkubermaticv1.ApplicationInstallationCleanupFinalizer) {
		if err := r.appInstaller.Delete(ctx, log, r.userClient, appInstallation); err != nil {
