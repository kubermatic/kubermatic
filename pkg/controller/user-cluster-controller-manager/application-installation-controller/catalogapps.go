...
type tolerationKey struct {
	...
	// chartDefaults are re-emitted because Helm replaces lists; omitting them drops the chart's own.
...
// applicationTolerationKeys maps an ApplicationDefinition to the values keys of its workloads'
// tolerations, verified against the newest pinned chart version. Helm ignores unknown values, so a
// renamed key silently stops being covered.
//
// aikit, k8sgpt-operator and kubevirt expose no toleration key and stay uncovered; they keep to
// untainted nodes. Covering them needs a Helm post-renderer.
...
// generateApplicationTolerationValues returns Helm values carrying the tolerations, or nil if the
// application is not covered. Merged with override, so each list keeps entries already set.
func generateApplicationTolerationValues(appName string, workloadTolerations []corev1.Toleration, currentValues map[string]any) map[string]any {
	...
	for _, key := range keys {
		// Chart defaults, then already-configured entries, so neither is lost when the list is replaced.
...
// currentTolerations reads the tolerations already at path. Malformed entries are skipped; the chart
// reports those, not KKP.
