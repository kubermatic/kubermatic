	...
	testCases := []struct {
	...
	}{
		{
			...
			validate: func(t *testing.T, catalog *catalogv1alpha1.ApplicationCatalog) {
				if catalog.Annotations != nil {
					if _, exists := catalog.Annotations["defaultcatalog.k8c.io/include"]; exists {
		...
		},
		{
			...
			validate: func(t *testing.T, catalog *catalogv1alpha1.ApplicationCatalog) {
				...
				annotation, exists := catalog.Annotations["defaultcatalog.k8c.io/include"]
				if !exists {
			...
			existingCatalog: &catalogv1alpha1.ApplicationCatalog{
				ObjectMeta: metav1.ObjectMeta{
					...
					Annotations: map[string]string{
						"defaultcatalog.k8c.io/include": "old-app-1,old-app-2",
					},
				},
			},
			config: &kubermaticv1.KubermaticConfiguration{
