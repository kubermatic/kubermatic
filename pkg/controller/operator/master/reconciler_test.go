...
func TestReconcileGatewayAPIResourcesKeepsManagedGatewayWhileLabeledHTTPRouteRejectedByExternalGateway(t *testing.T) {
	ctx := context.Background()
	namespace := "kubermatic"
	scheme := fake.NewScheme()
	externalNamespace := gatewayapiv1.Namespace("networking")
	...
	dexRoute := &gatewayapiv1.HTTPRoute{
		ObjectMeta: metav1.ObjectMeta{
			...
			Labels: map[string]string{
				common.NameLabel:                "dex",
				common.InstanceLabel:            "dex",
				common.GatewayHTTPRouteLabelKey: common.GatewayHTTPRouteLabelValue,
				modifier.ManagedByLabel:         "helm",
			},
		},
		Spec: gatewayapiv1.HTTPRouteSpec{
			CommonRouteSpec: gatewayapiv1.CommonRouteSpec{
	...
	if result.RequeueAfter != externalGatewayReadinessRequeueAfter {
		t.Fatalf("expected requeue after %s while Dex route is rejected by the external Gateway, got %s", externalGatewayReadinessRequeueAfter, result.RequeueAfter)
	}

	var gateway gatewayapiv1.Gateway
	if err := client.Get(ctx, types.NamespacedName{Namespace: namespace, Name: defaulting.DefaultGatewayName}, &gateway); err != nil {
		t.Fatalf("expected operator-managed Gateway to remain while Dex route is rejected, got %v", err)
	}
}
