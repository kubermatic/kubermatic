...
func apiPodLabels() map[string]string {
	return map[string]string{
		operatorresources.NameLabel: APIDeploymentName,
	}
}
	...
	return func() (string, reconciling.DeploymentReconciler) {
		return APIDeploymentName, func(d *appsv1.Deployment) (*appsv1.Deployment, error) {
			...
			args := []string{
				...
				fmt.Sprintf("-feature-gates=%s", operatorresources.StringifyFeatureGates(cfg)),
			...
			d.Spec.Template.Spec.Containers = []corev1.Container{
				{
					...
					Args:    args,
					Env:     operatorresources.KubermaticProxyEnvironmentVars(cfg.Spec.Proxy),
