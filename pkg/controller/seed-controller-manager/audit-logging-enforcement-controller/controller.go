...
func auditLoggingSettingsChanged(old, new *kubermaticv1.AuditLoggingSettings) bool {
	...
	if old.PolicyPreset != new.PolicyPreset {
		return true
	}
	// Use reflect.DeepEqual for complex nested structs (SidecarSettings, WebhookBackend)
	if !reflect.DeepEqual(old.SidecarSettings, new.SidecarSettings) {
		return true
	}
	if !reflect.DeepEqual(old.WebhookBackend, new.WebhookBackend) {
		return true
	}
...
func (r *reconciler) reconcile(ctx context.Context, cluster *kubermaticv1.Cluster, log *zap.SugaredLogger) error {
	...
	// Skip if enforcement is not enabled for this datacenter
	if !datacenter.Spec.EnforceAuditLogging {
		log.Debug("Audit logging enforcement not enabled for datacenter, skipping")
		return nil
	}

	// Check if cluster's audit logging configuration matches seed's configuration
	...
	// If you want to explicitly disable audit logging, set Enabled: false in the seed
	if seedAuditLogging == nil {
		log.Debug("Seed has no audit logging configuration, skipping enforcement")
		return nil
	}

	if reflect.DeepEqual(seedAuditLogging, clusterAuditLogging) {
		log.Debug("Cluster audit logging configuration already matches seed, no update needed")
		return nil
	}

	// Update cluster's audit logging configuration to match seed
	log.Infow("Enforcing audit logging configuration from seed", "seed", seed.Name)

	oldCluster := cluster.DeepCopy()
	cluster.Spec.AuditLogging = seedAuditLogging
	...
	log.Info("Successfully enforced audit logging configuration")
	return nil
}
