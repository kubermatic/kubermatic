...
func (r *Reconciler) GetSecretCreators(data *resources.TemplateData) []reconciling.NamedSecretCreatorGetter {
	...
	if data.IsKonnectivityEnabled() {
		creators = append(creators,
			...
			resources.GetInternalKubeconfigCreator(namespace, resources.KonnectivityKubeconfigSecretName, resources.KonnectivityKubeconfigUsername, nil, data, r.log),
		)
	} else {
		creators = append(creators,
			openvpn.CACreator(),
...
func (r *Reconciler) ensureNetworkPolicies(ctx context.Context, c *kubermaticv1.Cluster, data *resources.TemplateData) error {
	if c.Spec.Features[kubermaticv1.ApiserverNetworkPolicy] {
		...
		if data.IsKonnectivityEnabled() {
			namedNetworkPolicyCreatorGetters = append(namedNetworkPolicyCreatorGetters, apiserver.ApiserverInternalAllowReconciler())
		} else {
