...
func (r *Reconciler) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	if err != nil {
		if suppressedError != nil {
			...
			r.recorder.Eventf(backupConfig, corev1.EventTypeWarning, "ReconcilingError", "reconciling failed: %v", err)
			r.recorder.Eventf(cluster, corev1.EventTypeWarning, "ReconcilingError", "failed to reconcile etcd backup config %q: %v", backupConfig.Name, err)
		}
	}
