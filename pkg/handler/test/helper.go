...
func initTestEndpoint(user apiv1.User, seedsGetter provider.SeedsGetter, kubeObjects, machineObjects, kubermaticObjects []ctrlruntimeclient.Object, kubermaticConfiguration *kubermaticv1.KubermaticConfiguration, routingFunc newRoutingFunc) (http.Handler, *ClientsSets, error) {
	...
	operatingSystemProfileProviders := map[string]provider.PrivilegedOperatingSystemProfileProvider{"us-central1": privilegedOperatingSystemProfileProvider}
	operatingSystemProfileProviderGetter := func(seed *kubermaticv1.Seed) (provider.PrivilegedOperatingSystemProfileProvider, error) {
		if operatingSystemProfiles, exists := operatingSystemProfileProviders[seed.Name]; exists {
	...
	mainRouter := routingFunc(
		...
		operatingSystemProfileProviderGetter,
		featureGates,
	)
