	...
	testcases := []struct {
	...
	}{
		{
			...
			dcSpec: apiv1.DatacenterSpec{
				...
				EnabledOperatingSystems:  []string{"ubuntu", "centos"},
			},
			dcName:           "private-do1",
			dcPathName:       "private-do1",
			seedName:         "us-central1",
			expectedResponse: `{"metadata":{"name":"private-do1"},"spec":{"seed":"us-central1","country":"NL","location":"Amsterdam","digitalocean":{"region":"EU"},"node":{"pause_image":"pause-image"},"requiredEmailDomains":["example.com","pleaseno.org"],"enforceAuditLogging":true,"enforcePodSecurityPolicy":false,"enabledOperatingSystems":["ubuntu","centos"]}}`,
			httpStatus:       200,
			existingAPIUser:  test.GenDefaultAdminAPIUser(),
		},
		{
		...
		{
			name: "should receive a validation error when providing unsupported operating system for the dc",
			dcSpec: apiv1.DatacenterSpec{
				Seed:     "us-central1",
				Country:  "NL",
				Location: "Amsterdam",
				...
				EnforcePodSecurityPolicy: false,
				RequiredEmailDomains:     []string{"example.com", "pleaseno.org"},
				EnabledOperatingSystems:  []string{"ubuntu", "test"}, // "test" is not a Kubermatic supported OS.
			},
			dcName:           "private-do1",
			dcPathName:       "private-do1",
			seedName:         "us-central1",
			expectedResponse: `{"error":{"code":400,"message":"Validation error: EnabledOperatingSystems contains unsupported OS. Problematic OS was: test"}}`,
			httpStatus:       400,
			existingAPIUser:  test.GenDefaultAdminAPIUser(),
		},
	}
