...
func TestListNodeDeployments(t *testing.T) {
	...
	testcases := []struct {
		Name                       string
		ExpectedResponse           []apiv1.MachineDeployment
		HTTPStatus                 int
		ProjectIDToSync            string
		ClusterIDToSync            string
		ExistingProject            *kubermaticv1.Project
					...
					Spec: apiv1.MachineDeploymentSpec{
						Template: apiv1.MachineSpec{
							Cloud: apiv1.MachineCloudSpec{
								Digitalocean: &apiv1.DigitaloceanMachineSpec{
									Size: "2GB",
								},
							},
	...
	for _, tc := range testcases {
		t.Run(tc.Name, func(t *testing.T) {
			...
			actualNodeDeployments := machineDeploymentSliceWrapper{}
			actualNodeDeployments.DecodeOrDie(res.Body, t).Sort()

			wrappedExpectedNodeDeployments := machineDeploymentSliceWrapper(tc.ExpectedResponse)
...
// machineDeploymentSliceWrapper wraps []apiv1.MachineDeployment
// to provide convenient methods for tests
type machineDeploymentSliceWrapper []apiv1.MachineDeployment

// Sort sorts the collection by CreationTimestamp
func (k machineDeploymentSliceWrapper) Sort() {
...
func (k *machineDeploymentSliceWrapper) DecodeOrDie(r io.Reader, t *testing.T) *machineDeploymentSliceWrapper {
	t.Helper()
...
func (k machineDeploymentSliceWrapper) EqualOrDie(expected machineDeploymentSliceWrapper, t *testing.T) {
	t.Helper()
