...
func (r createPresetReq) Validate() error {
	...
	if hasProvider, _ := helper.HasProvider(convertAPIToInternalPreset(r.Body), crdapiv1.ProviderType(r.ProviderName)); !hasProvider {
		return fmt.Errorf("missing provider configuration for: %s", r.ProviderName)
	}

	err := helper.Validate(convertAPIToInternalPreset(r.Body), crdapiv1.ProviderType(r.ProviderName))
	if err != nil {
		return err
	}
	...
	for _, providerType := range crdapiv1.SupportedProviders {
		...
		if hasProvider, _ := helper.HasProvider(convertAPIToInternalPreset(r.Body), providerType); hasProvider {
...
func CreatePreset(presetProvider provider.PresetProvider, userInfoGetter provider.UserInfoGetter) endpoint.Endpoint {
	return func(ctx context.Context, request interface{}) (interface{}, error) {
		...
		if k8serrors.IsNotFound(err) {
			return presetProvider.CreatePreset(convertAPIToInternalPreset(req.Body))
		}
		...
		preset = mergePresets(preset, convertAPIToInternalPreset(req.Body), crdapiv1.ProviderType(req.ProviderName))
...
func convertAPIToInternalPreset(preset v2.PresetBody) *crdapiv1.Preset {
	return &crdapiv1.Preset{
