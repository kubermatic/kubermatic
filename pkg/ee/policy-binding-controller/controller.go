...
const (
	...
	cleanupFinalizer = kubermaticv1.PolicyBindingCleanupFinalizer
	...
	builderCtrl := builder.ControllerManagedBy(userMgr).
		...
		WatchesRawSource(source.Kind(seedMgr.GetCache(), &kubermaticv1.PolicyBinding{},
			...
			predicate.Or(
				...
				policyBindingDeletionTimestampChangedPredicate(),
			),
		)).
		WatchesRawSource(source.Kind(seedMgr.GetCache(), &kubermaticv1.Cluster{},
			handler.TypedEnqueueRequestsFromMapFunc(mapClusterToPolicyBindings(r.seedClient, namespace, clusterName, r.log)),
			policyBindingRelevantClusterChangedPredicate(clusterName),
		)).
...
func policyBindingDeletionTimestampChangedPredicate() predicate.TypedPredicate[*kubermaticv1.PolicyBinding] {
	return predicate.TypedFuncs[*kubermaticv1.PolicyBinding]{
		UpdateFunc: func(e ctrlruntimeevent.TypedUpdateEvent[*kubermaticv1.PolicyBinding]) bool {
...
func policyBindingRelevantClusterChangedPredicate(clusterName string) predicate.TypedPredicate[*kubermaticv1.Cluster] {
	return predicate.TypedFuncs[*kubermaticv1.Cluster]{
		...
		UpdateFunc: func(e ctrlruntimeevent.TypedUpdateEvent[*kubermaticv1.Cluster]) bool {
			...
			if e.ObjectOld.Spec.IsKyvernoEnabled() != e.ObjectNew.Spec.IsKyvernoEnabled() {
				return true
			}
...
func (r *reconciler) reconcile(ctx context.Context, log *zap.SugaredLogger, binding *kubermaticv1.PolicyBinding, cluster *kubermaticv1.Cluster) error {
	...
	if !cluster.Spec.IsKyvernoEnabled() {
		return r.markBindingInactiveAndCleanup(ctx, binding,
			kubermaticv1.PolicyBindingReasonKyvernoDisabled,
			"Kyverno resources have been deleted because Kyverno is disabled",
			"PolicyBinding is not active because Kyverno is disabled",
		)
	}

	if cluster.GetDeletionTimestamp() != nil {
		return r.markBindingInactiveAndCleanup(ctx, binding,
			kubermaticv1.PolicyBindingReasonDeleting,
			"Kyverno resources have been deleted because the cluster is being deleted",
			"PolicyBinding is not active because the cluster is being deleted",
		)
	}
...
// mapClusterToPolicyBindings maps cluster Kyverno state changes to all PolicyBindings in the cluster namespace.
func mapClusterToPolicyBindings(seedClient ctrlruntimeclient.Client, policyBindingNamespace, clusterName string, log *zap.SugaredLogger) func(ctx context.Context, cluster *kubermaticv1.Cluster) []reconcile.Request {
	return func(ctx context.Context, cluster *kubermaticv1.Cluster) []reconcile.Request {
		...
		if err := seedClient.List(ctx, bindings, ctrlruntimeclient.InNamespace(policyBindingNamespace)); err != nil {
			log.Error("Failed to list PolicyBindings to map Cluster change", "cluster", cluster.Name, "error", err)
			return nil
		}
...
// markBindingInactiveAndCleanup records why a binding is inactive, then removes
// generated Kyverno resources if the binding still owns the cleanup finalizer.
func (r *reconciler) markBindingInactiveAndCleanup(ctx context.Context, binding *kubermaticv1.PolicyBinding, reason, appliedMessage, readyMessage string) error {
	binding.SetCondition(kubermaticv1.PolicyBindingConditionKyvernoPolicyApplied, metav1.ConditionFalse, reason, appliedMessage)
	binding.SetCondition(kubermaticv1.PolicyBindingConditionReady, metav1.ConditionFalse, reason, readyMessage)
	binding.SetStatusFields(nil, false)
