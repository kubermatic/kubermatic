...
func CreateOrUpdateCredentialSecretForCluster(ctx context.Context, seedClient ctrlruntimeclient.Client, cluster *kubermaticv1.Cluster) error {
	...
	case cluster.Spec.Cloud.AWS != nil:
		_, err = createOrUpdateAWSSecret(ctx, seedClient, cluster)
	case cluster.Spec.Cloud.Azure != nil:
		_, err = createOrUpdateAzureSecret(ctx, seedClient, cluster)
	case cluster.Spec.Cloud.Digitalocean != nil:
		_, err = createOrUpdateDigitaloceanSecret(ctx, seedClient, cluster)
	case cluster.Spec.Cloud.GCP != nil:
		_, err = createOrUpdateGCPSecret(ctx, seedClient, cluster)
	case cluster.Spec.Cloud.Hetzner != nil:
		_, err = createOrUpdateHetznerSecret(ctx, seedClient, cluster)
	case cluster.Spec.Cloud.Openstack != nil:
		_, err = createOrUpdateOpenstackSecret(ctx, seedClient, cluster)
	case cluster.Spec.Cloud.Packet != nil:
		_, err = createOrUpdatePacketSecret(ctx, seedClient, cluster)
	case cluster.Spec.Cloud.Kubevirt != nil:
		_, err = createOrUpdateKubevirtSecret(ctx, seedClient, cluster)
	case cluster.Spec.Cloud.VSphere != nil:
		_, err = createVSphereSecret(ctx, seedClient, cluster)
	case cluster.Spec.Cloud.Alibaba != nil:
		_, err = createAlibabaSecret(ctx, seedClient, cluster)
	case cluster.Spec.Cloud.Anexia != nil:
		_, err = createOrUpdateAnexiaSecret(ctx, seedClient, cluster)
	case cluster.Spec.Cloud.Nutanix != nil:
		_, err = createOrUpdateNutanixSecret(ctx, seedClient, cluster)
	case cluster.Spec.Cloud.VMwareCloudDirector != nil:
		_, err = createOrUpdateVMwareCloudDirectorSecret(ctx, seedClient, cluster)
	}
...
func createOrUpdateOpenstackSecret(ctx context.Context, seedClient ctrlruntimeclient.Client, cluster *kubermaticv1.Cluster) (bool, error) {
	...
	credentialRef, err := ensureCredentialSecret(ctx, seedClient, cluster, map[string][]byte{
		...
		resources.OpenstackToken:                       []byte(spec.Token),
	})
	if err != nil {
		return false, err
	}
	...
	cluster.Spec.Cloud.Openstack.Token = ""
