	...
	testcases := []testcase[kubevirt.RawConfig]{
		&kubevirtTestcase{
			baseTestcase: baseTestcase[kubevirt.RawConfig, kubermaticv1.DatacenterSpecKubevirt]{
				...
				expected: cloneBuilder(goodMachine).WithDNSPolicy("test-dnspolicy").WithClusterName("testcluster"),
			},
		},
		&kubevirtTestcase{
			baseTestcase: baseTestcase[kubevirt.RawConfig, kubermaticv1.DatacenterSpecKubevirt]{
				...
				expected:  cloneBuilder(goodMachine).WithDNSPolicy("keep-me-kubevirt").WithClusterName("testcluster"),
			},
		},
	}
