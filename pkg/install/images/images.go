...
func getImagesFromReconcilers(log logrus.FieldLogger, templateData *resources.TemplateData, config *kubermaticv1.KubermaticConfiguration, kubermaticVersions kubermatic.Versions) (images []string, err error) {
	...
	deploymentReconcilers := controlplanecontroller.GetDeploymentReconcilers(templateData, false, kubermaticVersions)
	...
	if templateData.IsKonnectivityEnabled() {
		deploymentReconcilers = append(deploymentReconcilers, konnectivity.DeploymentReconciler(templateData.Cluster().Spec.Version, "dummy", 0, resources.DefaultKonnectivityKeepaliveTime, registry.GetImageRewriterFunc(templateData.OverwriteRegistry)))
	}
