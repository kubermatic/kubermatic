...
func TestValidateConfigurationDefaultsSharedIAPGatewayValues(t *testing.T) {
	doc := userClusterMLAIAPValues(t)

	mlaStack := &UserClusterMLAStack{}
	_, gotDoc, failures := mlaStack.ValidateConfiguration(userClusterMLAExternalGatewayConfig(), doc, stack.DeployOptions{MLAIncludeIap: true}, logrus.New())
	if len(failures) > 0 {
...
func TestValidateConfigurationPreservesSeparateSeedIAPGatewayValues(t *testing.T) {
	...
	mlaStack := &UserClusterMLAStack{}
	_, gotDoc, failures := mlaStack.ValidateConfiguration(userClusterMLAExternalGatewayConfig(), doc, stack.DeployOptions{MLAIncludeIap: true, SeparateSeed: true}, logrus.New())
	if len(failures) > 0 {
		t.Fatalf("expected no validation failures, got %v", failures)
	}
...
func userClusterMLAIAPValues(t *testing.T) *yamled.Document {
	t.Helper()
...
func userClusterMLAExternalGatewayConfig() *kubermaticv1.KubermaticConfiguration {
...
func assertIAPGatewayValues(t *testing.T, doc *yamled.Document, wantName, wantNamespace string) {
	...
	if gatewayName != wantName {
		t.Fatalf("expected usercluster MLA IAP Gateway name %s, got %s", wantName, gatewayName)
	}

	gatewayNamespace, _ := doc.GetString(yamled.Path{"httpRoute", "gatewayNamespace"})
	if gatewayNamespace != wantNamespace {
		t.Fatalf("expected usercluster MLA IAP Gateway namespace %s, got %s", wantNamespace, gatewayNamespace)
	}
}
