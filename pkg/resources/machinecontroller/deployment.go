...
func getEnvVars(data machinecontrollerData) ([]corev1.EnvVar, error) {
	...
	if data.Cluster().Spec.Cloud.Openstack != nil {
		...
		vars = append(vars, corev1.EnvVar{Name: "OS_TENANT_NAME", Value: credentials.Openstack.Project})
		vars = append(vars, corev1.EnvVar{Name: "OS_TENANT_ID", Value: credentials.Openstack.ProjectID})
