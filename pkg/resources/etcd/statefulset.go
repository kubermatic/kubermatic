...
func getEtcdCommand(cluster *kubermaticv1.Cluster, enableCorruptionCheck, launcherEnabled bool, quotaBackendGB int64) []string {
	...
	command := []string{
		...
		fmt.Sprintf("https://$(POD_NAME).%s.%s.svc.cluster.local:2379,https://127.0.0.1:2379", resources.EtcdServiceName, cluster.Status.NamespaceName),
		"--listen-client-urls",
		fmt.Sprintf("https://$(POD_NAME).%s.%s.svc.cluster.local:2379,https://$(POD_IP):2379,https://127.0.0.1:2379", resources.EtcdServiceName, cluster.Status.NamespaceName),
		"--listen-peer-urls",
		fmt.Sprintf("https://$(POD_NAME).%s.%s.svc.cluster.local:2380, https://$(POD_IP):2380", resources.EtcdServiceName, cluster.Status.NamespaceName),
		"--listen-metrics-urls",
		fmt.Sprintf("https://$(POD_NAME).%s.%s.svc.cluster.local:2378,https://$(POD_IP):2378,https://127.0.0.1:2378", resources.EtcdServiceName, cluster.Status.NamespaceName),
