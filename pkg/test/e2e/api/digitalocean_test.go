	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if err := testClient.WaitForClusterHealthy(project.ID, tc.dc, cluster.ID); err != nil {
				if err := testClient.PrintClusterEvents(project.ID, tc.dc, cluster.ID); err != nil {
			...
			if err := testClient.WaitForClusterNodeDeploymentsToByReady(project.ID, tc.dc, cluster.ID, tc.replicas); err != nil {
