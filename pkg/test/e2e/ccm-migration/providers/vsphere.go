...
type VsphereClusterJig struct {
	...
	VsphereCredentials VsphereCredentialsType
}

func (c *VsphereClusterJig) Setup() error {
	c.Log.Debugw("Setting up new cluster", "name", c.Name)

	if err := c.generateAndCreateSecret(c.VsphereCredentials.GenerateSecretData()); err != nil {
...
func (c *VsphereClusterJig) CreateMachineDeployment(userClient ctrlruntimeclient.Client) error {
	if err := c.generateAndCCreateMachineDeployment(userClient, c.VsphereCredentials.GenerateProviderSpec()); err != nil {
		return errors.Wrap(err, "failed to create machine deployment")
	}
	return nil
}

func (c *VsphereClusterJig) CheckComponents() bool {
	return true
}
