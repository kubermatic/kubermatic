...
func (c *VsphereClusterJig) ExposeAPIServer() error {
...
func (vc *VsphereCredentialsType) GenerateProviderSpec(clustername string) []byte {
	cloudProviderSpec := fmt.Sprintf(`{"allowInsecure":false,"cluster":"%s","cpus":2,"datacenter":"%s","datastore":"exsi-nas","datastoreCluster":"","diskSizeGB":10,"memoryMB":4096,"folder":"/%s/vm/kubermatic/%s","templateVMName":"machine-controller-e2e-ubuntu"}`, vc.Cluster, vc.Datacenter, vc.Datacenter, clustername)
