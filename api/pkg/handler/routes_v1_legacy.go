...
func (r Routing) getNodeForClusterLegacy() http.Handler {
	return httptransport.NewServer(
		...
		)(node.GetNodeForClusterLegacyEndpoint(r.projectProvider, r.userInfoGetter)),
...
func (r Routing) listNodesForClusterLegacy() http.Handler {
	return httptransport.NewServer(
		endpoint.Chain(
			...
			middleware.UserSaver(r.userProvider),
			middleware.SetClusterProvider(r.clusterProviderGetter, r.seedsGetter),
		)(node.ListNodesForClusterLegacyEndpoint(r.projectProvider, r.userInfoGetter)),
...
func (r Routing) deleteNodeForClusterLegacy() http.Handler {
	return httptransport.NewServer(
		endpoint.Chain(
			...
			middleware.UserSaver(r.userProvider),
			middleware.SetClusterProvider(r.clusterProviderGetter, r.seedsGetter),
		)(node.DeleteNodeForClusterLegacyEndpoint(r.projectProvider, r.userInfoGetter)),
