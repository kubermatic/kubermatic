...
func TestList(t *testing.T) {
	...
	testcases := []struct {
		...
		{
			name:          "scenario 1: list existing service accounts",
			projectToSync: "plan9-ID",
		...
		{
			name:          "scenario 2: list service accounts for project when binding is missing",
			projectToSync: "plan9-ID",
			httpStatus:    http.StatusInternalServerError,
			existingKubermaticObjs: []runtime.Object{
				/*add projects*/
			...
			existingAPIUser: *test.GenAPIUser("john", "john@acme.com"),
			expectedError:   `{"error":{"code":500,"message":"failed to get some service accounts, please examine details field for more info","details":["binding missing for test-1"]}}`,
		},
	}
