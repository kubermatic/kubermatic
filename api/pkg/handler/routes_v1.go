...
func (r Routing) createNodeDeployment() http.Handler {
	return httptransport.NewServer(
		endpoint.Chain(
			...
			middleware.SetClusterProvider(r.clusterProviderGetter, r.seedsGetter),
			middleware.UserInfoExtractor(r.userProjectMapper),
		)(node.CreateNodeDeployment(r.sshKeyProvider, r.projectProvider, r.credentialsProvider, r.seedsGetter)),
...
func (r Routing) patchNodeDeployment() http.Handler {
	return httptransport.NewServer(
		endpoint.Chain(
			...
			middleware.SetClusterProvider(r.clusterProviderGetter, r.seedsGetter),
			middleware.UserInfoExtractor(r.userProjectMapper),
		)(node.PatchNodeDeployment(r.sshKeyProvider, r.projectProvider, r.credentialsProvider, r.seedsGetter)),
...
func (r Routing) createAddon() http.Handler {
	return httptransport.NewServer(
		endpoint.Chain(
			...
			middleware.UserSaver(r.userProvider),
			middleware.SetClusterProvider(r.clusterProviderGetter, r.seedsGetter),
			middleware.Addons(r.addonProviderGetter),
			middleware.UserInfoExtractor(r.userProjectMapper),
