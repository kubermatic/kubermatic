...
func (r Routing) listOpenstackSizes() http.Handler {
	return httptransport.NewServer(
		endpoint.Chain(
			middleware.TokenVerifier(r.tokenVerifiers),
			middleware.UserSaver(r.userProvider),
		)(provider.OpenstackSizeEndpoint(r.cloudProviders, r.seeds, r.presetsManager)),
...
func (r Routing) createCluster(initNodeDeploymentFailures *prometheus.CounterVec) http.Handler {
	return httptransport.NewServer(
		endpoint.Chain(
			...
			middleware.UserInfoExtractor(r.userProjectMapper),
		)(cluster.CreateEndpoint(r.sshKeyProvider, r.cloudProviders, r.projectProvider, r.seeds, initNodeDeploymentFailures, r.eventRecorderProvider, r.presetsManager, r.exposeStrategy)),
...
func (r Routing) patchCluster() http.Handler {
	return httptransport.NewServer(
		endpoint.Chain(
			...
			middleware.SetClusterProvider(r.clusterProviders, r.seeds),
			middleware.UserInfoExtractor(r.userProjectMapper),
		)(cluster.PatchEndpoint(r.cloudProviders, r.projectProvider, r.seeds)),
...
func (r Routing) listOpenstackSizesNoCredentials() http.Handler {
	return httptransport.NewServer(
		endpoint.Chain(
			...
			middleware.SetClusterProvider(r.clusterProviders, r.seeds),
			middleware.UserInfoExtractor(r.userProjectMapper),
		)(provider.OpenstackSizeNoCredentialsEndpoint(r.projectProvider, r.cloudProviders, r.seeds)),
