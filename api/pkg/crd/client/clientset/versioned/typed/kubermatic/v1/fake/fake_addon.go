...
func (c *FakeAddons) Get(name string, options v1.GetOptions) (result *kubermatic_v1.Addon, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewGetAction(addonsResource, c.ns, name), &kubermatic_v1.Addon{})

	if obj == nil {
		return nil, err
	}
	return obj.(*kubermatic_v1.Addon), err
}

// List takes label and field selectors, and returns the list of Addons that match those selectors.
func (c *FakeAddons) List(opts v1.ListOptions) (result *kubermatic_v1.AddonList, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewListAction(addonsResource, addonsKind, c.ns, opts), &kubermatic_v1.AddonList{})
	...
	list := &kubermatic_v1.AddonList{}
	for _, item := range obj.(*kubermatic_v1.AddonList).Items {
...
func (c *FakeAddons) Create(addon *kubermatic_v1.Addon) (result *kubermatic_v1.Addon, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewCreateAction(addonsResource, c.ns, addon), &kubermatic_v1.Addon{})
...
func (c *FakeAddons) Update(addon *kubermatic_v1.Addon) (result *kubermatic_v1.Addon, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateAction(addonsResource, c.ns, addon), &kubermatic_v1.Addon{})
...
func (c *FakeAddons) Delete(name string, options *v1.DeleteOptions) error {
	_, err := c.Fake.
		Invokes(testing.NewDeleteAction(addonsResource, c.ns, name), &kubermatic_v1.Addon{})
...
func (c *FakeAddons) DeleteCollection(options *v1.DeleteOptions, listOptions v1.ListOptions) error {
	...
	_, err := c.Fake.Invokes(action, &kubermatic_v1.AddonList{})
	return err
}

// Patch applies the patch and returns the patched addon.
func (c *FakeAddons) Patch(name string, pt types.PatchType, data []byte, subresources ...string) (result *kubermatic_v1.Addon, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewPatchSubresourceAction(addonsResource, c.ns, name, data, subresources...), &kubermatic_v1.Addon{})
