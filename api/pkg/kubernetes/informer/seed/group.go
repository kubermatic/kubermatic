...
type Group struct {
	...
	RoleInformer               rbacv1beta1.RoleInformer
	RoleBindingInformer        rbacv1beta1.RoleBindingInformer
...
func New(kubeClient kubernetes.Interface, crdClient crdclient.Interface) *Group {
	...
	g.RoleInformer = coreInformers.Rbac().V1beta1().Roles()
	g.RoleBindingInformer = coreInformers.Rbac().V1beta1().RoleBindings()
...
func (g *Group) HasSynced() bool {
	return g.NamespaceInformer.Informer().HasSynced() &&
		...
		g.RoleInformer.Informer().HasSynced() &&
		g.RoleBindingInformer.Informer().HasSynced() &&
...
func (g *Group) Run(stopCh <-chan struct{}) {
	...
	go g.RoleInformer.Informer().Run(stopCh)
	go g.RoleBindingInformer.Informer().Run(stopCh)
