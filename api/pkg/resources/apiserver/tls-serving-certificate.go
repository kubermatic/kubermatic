...
func TLSServingCertificate(data *resources.TemplateData, existing *corev1.Secret) (*corev1.Secret, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to get cluster ip: %v", err)
	}
	...
	se.Data = map[string][]byte{
		resources.ApiserverTLSKeySecretKey:  certutil.EncodePrivateKeyPEM(key),
		resources.ApiserverTLSCertSecretKey: certutil.EncodeCertPEM(cert),
	}
