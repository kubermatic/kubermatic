...
func main() {
	...
	kubermaticFiles, err := filepath.Glob(filepath.Join(root, "vendor/k8c.io/kubermatic/v2/sdk/apis/kubermatic/v1/*.go"))
	if err != nil {
		log.Fatalf("Failed to find go files: %v", err)
	}

	appsKubermaticFiles, err := filepath.Glob(filepath.Join(root, "vendor/k8c.io/kubermatic/v2/sdk/apis/apps.kubermatic/v1/*.go"))
	if err != nil {
