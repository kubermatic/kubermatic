...
for key in "${KEYS[@]}" ; do
    ...
    for dash in $(fetch_fields ${key} 'search?query=&' 'uri'); do
        ...
        echo $DB
        curl -f -k -H "Authorization: Bearer ${key}" "${HOST}/api/dashboards/${dash}" | jq '.dashboard.id = null' | jq '.overwrite = true' > "$DIR/dashboard-$DB"
    done

    for id in $(fetch_fields ${key} 'datasources' 'id'); do
        DS=$(echo $(fetch_fields ${key} "datasources/${id}" 'name')|sed 's/ /-/g').json
        echo $DS
        curl -f -k -H "Authorization: Bearer ${key}" "${HOST}/api/datasources/${id}" | jq '.id = null' | jq '.orgId = null' > "$DIR/datasource-$DS"
    done
