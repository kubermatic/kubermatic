...
import (
	...
	"github.com/kubermatic/kubermatic/bare-metal-provider/pkg/api/handler"
...
type Server struct {
	...
	clusterController cache.ControllerInterface
...
func (s *Server) RemoveDeadNodes() {
	...
	for _, node := range nodes {
		...
		err := s.client.Nodes(handler.Namespace).Delete(node.Metadata.Name, &v1.DeleteOptions{})
		if err != nil {
...
func New(address string, c extensions.Clientset, authUser string, authPass string) *Server {
	...
	nodeStore, nodeController := cache.NewInformer(
		&cache.ListWatch{
			ListFunc: func(options api.ListOptions) (runtime.Object, error) {
				return s.client.Nodes(v1.NamespaceAll).List(options)
			},
			WatchFunc: func(options api.ListOptions) (watch.Interface, error) {
				return s.client.Nodes(v1.NamespaceAll).Watch(options)
			},
		},
		&extensions.Node{},
		10*time.Minute,
	...
	clusterStore, clusterController := cache.NewInformer(
		&cache.ListWatch{
			ListFunc: func(options api.ListOptions) (runtime.Object, error) {
				return s.client.Clusters(v1.NamespaceAll).List(options)
			},
			WatchFunc: func(options api.ListOptions) (watch.Interface, error) {
				return s.client.Clusters(v1.NamespaceAll).Watch(options)
			},
		},
		&extensions.Cluster{},
		10*time.Minute,
